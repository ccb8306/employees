	...
	<div>
		...
		<div>
			<%
				if(currentPage > 1) {
			%>
				<button id="btnPage" type="button" onclick="location.href='./deptManagerList.jsp?currentPage=1'">처음으로</button>
				<button id="btnPage" type="button" onclick="location.href='./deptManagerList.jsp?currentPage=<%=currentPage -1 %>'">이전</button>
			<%
				}
			%>
			<%
				if(currentPage < cnt) {
			%>
				<button id="btnPage" type="button" onclick="location.href='./deptManagerList.jsp?currentPage=<%=currentPage +1 %>'">다음</button>
				<button id="btnPage" type="button" onclick="location.href='./deptManagerList.jsp?currentPage=<%=cnt %>'">마지막으로</button>
			<%
				}
			%>
		</div>
